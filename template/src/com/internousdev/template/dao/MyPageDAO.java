...
public class MyPageDAO {
	public MyPageDTO getMyPageUserInfo(String item_transaction_id, String user_master_id) throws SQLException {
		...
		String sql = "SELECT iit.item_name, ubit.total_price, ubit.total_count, ubit.pay "
				...
				+ "LEFT JOIN item_info_transaction iit "	//基準となるテーブル(主キー(primary key)を宣言する)
				+ "ON ubit.item_transaction_id = iit.id "	//上のテーブルと結合される
				+ "WHERE ubit.item_transaction_id = ? "		//選出条件1
				+ "AND ubit.user_master_id = ? "			//選出条件2
				+ "ORDER BY ubit.insert_date DESC";			//降順(desc)で表示
